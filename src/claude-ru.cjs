...
const translations = {
  ...
  'Set up a long-lived authentication token': 'Настроить постоянный токен аутентификации',
  'requires Claude subscription': 'требуется подписка Claude',
  'Check the health of your Claude Code auto-updater': 'Проверить автообновление Claude Code',
  'Check for updates and install if available': 'Проверить наличие обновлений и установить при наличии',
  'Install Claude Code native build. Use [target] to specify version': 'Установить нативную сборку Claude Code. [target] - версия',
  'stable, latest, or specific version': 'stable, latest, или конкретная версия',

  // Options descriptions - detailed
  'Enable debug mode with optional category filtering': 'Включить отладку с фильтрацией категорий',
  'Override verbose mode setting from config': 'Переопределить подробный режим из конфига',
  'Print response and exit': 'Вывести ответ и выйти',
  'useful for pipes': 'полезно для конвейеров',
  'Note: The workspace trust dialog is skipped when Claude is run with the -p mode. Only use this flag in directories you trust.': 'Примечание: В режиме -p диалог доверия пропускается. Используйте только в доверенных директориях.',
  ...
  'or open interactive picker with optional search term': 'или открыть интерактивный выбор с поиском',
  'Include partial message chunks as they arrive': 'Включить фрагменты сообщений по мере поступления',
  'Enable MCP debug mode': 'Включить режим отладки MCP',
  'shows MCP server errors': 'показывает ошибки MCP сервера',
  'Recommended only for sandboxes with no internet access.': 'Только для песочниц без интернета.',
  ...
  'Re-emit user messages from stdin back on stdout for acknowledgment': 'Дублировать сообщения из stdin в stdout для подтверждения',
  'Comma or space-separated list of tool names to allow': 'Разрешенные инструменты (через запятую или пробел)',
  'Comma or space-separated list of tool names to deny': 'Запрещенные инструменты (через запятую или пробел)',
  'Specify the list of available tools from the built-in set.': 'Укажите список доступных инструментов из встроенного набора.',
  'Use "" to disable all tools, "default" to use all tools, or specify tool names': '"" - отключить все, default - все инструменты, или укажите имена',
  ...
  'When resuming, create a new session ID instead of reusing the original': 'При возобновлении создать новый ID сессии',
  'use with --resume or --continue': 'использовать с --resume или --continue',
  'Disable session persistence - sessions will not be saved to disk and cannot be resumed': 'Отключить сохранение сессий - они не сохраняются и не возобновляются',
  ...
  'Enable automatic fallback to specified model when default model is overloaded': 'Автопереход на указанную модель при перегрузке основной',
  ...
  'Additional directories to allow tool access to': 'Дополнительные директории для доступа инструментов',
  'Automatically connect to IDE on startup if exactly one valid IDE is available': 'Автоподключение к IDE при запуске (если доступна одна)',
  'Only use MCP servers from --mcp-config, ignoring all other MCP configurations': 'Только MCP серверы из --mcp-config, игнорируя остальные',
...
if (isInteractive && !args.includes('-p') && !args.includes('--print')) {
  // RGB градиент от синего к фиолетовому к розовому
  const rgb1 = '\x1b[38;2;0;150;255m';      // Яркий синий
  const rgb2 = '\x1b[38;2;100;100;255m';    // Сине-фиолетовый
  const rgb3 = '\x1b[38;2;150;50;255m';     // Фиолетовый
  const rgb4 = '\x1b[38;2;200;50;200m';     // Розово-фиолетовый
  const rgb5 = '\x1b[38;2;255;100;150m';    // Розовый
  const rgb6 = '\x1b[38;2;255;150;100m';    // Коралловый
  const reset = '\x1b[0m';
  const bold = '\x1b[1m';
  const green = '\x1b[38;2;50;255;100m';    // Яркий зеленый
  const gray = '\x1b[38;2;150;150;150m';    // Серый

  console.log(rgb1 + '╔' + rgb2 + '═══════════════════' + rgb3 + '═══════════════════' + rgb4 + '═══════════════════' + rgb5 + '═══╗' + reset);
  console.log(rgb1 + '║' + reset + '  ' + bold + rgb2 + '🇷🇺 Claude Code' + rgb3 + ' - ' + rgb4 + 'Русская версия' + reset + '                        ' + rgb5 + '║' + reset);
  console.log(rgb1 + '║' + reset + '                                                            ' + rgb6 + '║' + reset);
  console.log(rgb2 + '║' + reset + '  ' + green + '✨ Спасибо, что используете русскую локализацию!' + reset + '       ' + rgb6 + '║' + reset);
  console.log(rgb2 + '║' + reset + '  ' + gray + '470+ переведенных фраз | Поддержка PTY' + reset + '                ' + rgb5 + '║' + reset);
  console.log(rgb3 + '║' + reset + '                                                            ' + rgb5 + '║' + reset);
  console.log(rgb3 + '║' + reset + '  ' + gray + 'GitHub: ' + rgb4 + 'github.com/logansin/claude-code-ru' + reset + '            ' + rgb4 + '║' + reset);
  console.log(rgb4 + '╚' + rgb5 + '═══════════════════' + rgb6 + '═══════════════════' + rgb1 + '═══════════════════' + rgb2 + '═══╝' + reset);
  console.log('');
}
